...
Set them as env vars on the api Container App (either by editing
`azure/bicep/modules/apps.bicep` and redeploying, or via
`az containerapp update --set-env-vars`). The api re-reads them on
revision restart.
...
2. Reuse the `HEADER` constant so you get the `$schema` and elevated security
   context for free.
3. Rebuild and push the web image: `azure/scripts/build-and-push.ps1
   -ImagesToBuild web`, then `az containerapp update -n web` (or just
   re-run `azure/scripts/deploy-apps.ps1`).
...
- **Postgres credentials** — the admin password is generated by
  `azure/scripts/deploy-apps.ps1` and stored as the `pg-admin-password`
  Container Apps secret + the `pgPassword` field in
  `.azure/secrets.local.json`. Rotate by deleting the secret line in
  `secrets.local.json` and re-running `deploy-apps.ps1` (it will mint a
  fresh password and update both the Flexible Server and the api env).
- **API port** — `API_PORT` (default `3000`). The web nginx proxy and
  the ACA ingress target this port.
